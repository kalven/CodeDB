...
namespace {
void add_single(std::map<std::string, std::string>& options,
                const std::string& opt, const std::string value = "") {
...
options parse_cmdline(int argc, char** argv) {
  ...
  if (args.empty() || args[0] == "-h" || args[0] == "--help" ||
    ...
    if (args.size() == 2) result.m_args.push_back(args[1]);
    if (args.size() > 2) throw std::runtime_error("Invalid argument");
  } else if (args[0] == "init") {
    result.m_mode = options::init;
    if (args.size() > 1) throw std::runtime_error("Invalid argument");
    ...
    if (i != args.end()) throw std::runtime_error("Invalid argument");
  } else if (args[0] == "config") {
    result.m_mode = options::config;
    if (args.size() > 3) throw std::runtime_error("Invalid argument");
