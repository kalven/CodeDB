...
void validate_regex(const std::string& value) { compile_regex(value); }

void validate_bool(const std::string& value) {
  if (value != "on" && value != "off")
    throw std::runtime_error(
        "'" + value + "' is not a valid boolean, expected 'on' or 'off'");
}
...
void validate_threads(const std::string& value) {
  ...
  if (!re->match(value))
    throw std::runtime_error(
        "'" + value + "' is not valid, expected 'default' or an integer");
}
...
const config_keys& get_config_map() {
  ...
  if (keys.empty()) {
    keys.insert(std::make_pair(
        "dir-exclude",
        cfg_key("(\\.codedb|\\.git|\\.svn|\\.hg|_darcs)", &validate_regex)));
    keys.insert(std::make_pair(
        "file-include", cfg_key(".*?\\.(h|hpp|inl|c|cpp)", &validate_regex)));
    keys.insert(
        std::make_pair("nocase-file-match", cfg_key("off", &validate_bool)));
    ...
    keys.insert(
        std::make_pair("find-threads", cfg_key("default", &validate_threads)));
  }
...
void config::load(std::istream& in) {
  ...
  while (getline(in, line)) {
    if (re->match(line)) m_cfg.insert(std::make_pair(re->what(1), re->what(2)));
  }
}
...
std::string config::get_value(const std::string& name) const {
  ...
  if (i == m_cfg.end()) return key.m_default;
  return i->second;
}
...
void run_config(const bfs::path& cdb_path, const options& opt) {
  ...
  switch (opt.m_args.size()) {
    case 0: {
      ...
      for (auto i = keys.begin(); i != keys.end(); ++i) {
        std::cout << std::left << std::setw(max_size) << i->first << " = "
                  << cfg.get_value(i->first) << std::endl;
      }
      break;
    }
    case 1: {
