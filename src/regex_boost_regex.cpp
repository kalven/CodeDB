...
boost::regex_constants::syntax_option_type boost_regex_options(
    const char* flags) {
  // Default options
  ...
  for (; *flags; ++flags) {
    if (*flags == 'i') res = res | boost::regex_constants::icase;
  }
...
regex_ptr compile_regex_boost_regex(const std::string& expr, int,
                                    const char* flags) {
  try {
