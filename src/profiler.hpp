...
inline void profile_start(profiler& p) { p.m_start = getticks(); }
...
class profile_scope {
 public:
  profile_scope(profiler& p) : m_p(p) { profile_start(m_p); }

  profile_scope(const char* name) : m_p(make_profiler(name)) {
    profile_start(m_p);
  }
