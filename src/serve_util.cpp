...
inline char hexchar(int x) { return x < 10 ? '0' + x : 'A' + (x - 10); }
...
std::string get_arg(const http_query& query, const std::string& index) {
  ...
  if (it == query.m_args.end()) return std::string();
...
std::vector<std::string> serve_file(const bfs::path& path) {
  if (!bfs::exists(path)) return serve_page("not found!");
  if (!bfs::is_regular(path)) return serve_page("not regular!");

  bfs::ifstream file(path);
  if (!file.is_open()) return serve_page("unable to open!");
